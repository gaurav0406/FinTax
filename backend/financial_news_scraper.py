...
DO NOT use introductory labels. The output must be crisp, concise, and direct.

Input Items:{json.dumps(simplified, indent=2)}

Respond ONLY with a JSON Array of objects matching this exact format for each item:
[
  {{
    "id": <number matching input id>,
    "title": "Ultra-crisp headline (Max 8 words)",
    "summary": "Brief 1-sentence executive summary.",
    "who_impacted": "Target audience.",
    "reason": "EXACTLY 3 bullet points. Each MUST NOT exceed 8 words. Focus on drivers.",
    "financial_impact": "EXACTLY 3 bullet points. Each MUST start with a number/percentage/KPI. Max 8 words.",
    "action": "EXACTLY 2 bullet points. Each MUST NOT exceed 8 words. Direct actions only.",
        ...
        for attempt in range(2):
            try:
                ...
                if e.code == 429:
                    ...
                    logging.error(f"Gemini API HTTP Error ({url}): {e}")
                    break
            except Exception as e:
